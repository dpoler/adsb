...
// Labels shown only when runways are wide enough to be unambiguous
#define RUNWAY_LABEL_MAX_NM  10.0f  // outer range gate
...
// Marks the current center reference point — home when Home is active,
// or the selected airport's reference point otherwise.
...
static void draw_runway_label(lv_layer_t *layer, lv_draw_label_dsc_t *lbl,
...
// Draws runway lines (+ designators, at close range) for the active saved
// airport. Home never has runway data, so this is a no-op while on Home.
static void draw_runways(lv_layer_t *layer) {
    ...
    float radius_nm = range_get_nm();
    bool show_labels = (radius_nm <= RUNWAY_LABEL_MAX_NM);
